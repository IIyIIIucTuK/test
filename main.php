...
for($i = 0; $i < $count; $i++) {
    switch ($i) {
        case 0:
            ...
            $final[] = $rsquare->getSquare() . ' rect ' . $rectangle->a . ' ' . $rectangle->b;
            break;
        case 1:
            $csquare = new circle($circle->radius);
            $final[] = $csquare->getSquare(). ' circle ' . $circle->radius;
            break;
        case 2:
            $tsquare = new triangle($triangle->a, $triangle->b, $triangle->c);
            $final[] = $tsquare->getSquare(). ' triangle ' . $triangle->a . ' ' . $triangle->b . ' ' . $triangle->c;
            break;
    }
}
//сортировка
